...
def eval_all(expressions, env, tail=False):
    ...
    if expressions is nil:
        ...
        while expressions.rest:
            scheme_eval(expressions.first, env, expressions.rest is nil)
...
def do_and_form(expressions, env):
    ...
    if not expressions:
        return True
    while expressions.rest:
        if is_false_primitive(scheme_eval(expressions.first, env, expressions.rest is nil)):
            return False
        expressions = expressions.rest
    return scheme_eval(expressions.first, env, expressions.rest is nil)
    # END PROBLEM 12
