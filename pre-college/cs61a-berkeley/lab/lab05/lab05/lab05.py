...
def couple(lst1, lst2):
    ...
    "*** YOUR CODE HERE ***"
    return [[lst1[index1], lst2[index1]] for index1 in range(len(lst1))]
...
def closer_city(lat, lon, city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    middleman = make_city('Middleman', lat, lon)
    if distance(city1, middleman) > distance(city2, middleman):
...
def nut_finder(t):
    ...
    if label(t) == 'nut':
        ...
        branch_nuts = [nut_finder(branch) for branch in branches(t)]
        return True in branch_nuts
...
def sprout_leaves(t, values):
    ...
    if is_leaf(t):
        ...
        sprout_leaf_branch = [sprout_leaves(branch, values) for branch in branches(t)]
        return tree(label(t), sprout_leaf_branch)
...
def add_chars(w1, w2):
    ...
    if w1 == '':
        ...
        return add_chars(w1.replace(w1[0], '', 1), w2.replace(w1[0], '', 1))
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) and is_leaf(t2):
        ...
        zipped_branches = zip_longest(branches(t1), branches(t2), fillvalue = tree(0))
        added_branches = [add_trees(b1, b2) for b1, b2 in zipped_branches]
